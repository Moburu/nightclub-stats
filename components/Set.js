export default function Set(props) {
    ...
    const was_set_dq = (p1_score === -1 || p2_score === -1);
    ...
    return (
        <div>
            {
                (was_set_dq && did_hero_win) ? (
                    <div className="grid grid-cols-8 grid-rows-2">
                        <span className="col-span-8">{round_text}</span>
                        <span className="col-span-3 bg-linear-to-r from-none via-gray-1000 to-gray-600">{winner_tag}</span>
                        <span className="col-span-1 bg-gray-600">{winner_score}</span>
                        <span className="col-span-1">{loser_score}</span>
                        <span className="col-span-3">{loser_tag}</span>
                    </div>
                )
                : (was_set_dq && !did_hero_win) ? (
                    <div className="grid grid-cols-8 grid-rows-2">
                        ...
                        <span className="col-span-1">{loser_score}</span>
                        <span className="col-span-1 bg-gray-600">{winner_score}</span>
                        <span className="col-span-3 bg-linear-to-l from-none via-gray-1000 to-gray-600">{winner_tag}</span>
                    </div>
                )
                : did_hero_win ? (
                    <div className="grid grid-cols-8 grid-rows-2">
